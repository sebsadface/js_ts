    ...
    \begin{part}[a]
       ...
       \begin{part}[h]
        \begin{answer}
            ...
            in index.tsx, we need exhaustively test all 5 patterns with and without "symmetrize" request (10 possible cases total). 
...
\begin{question}{6. Quilt To Last (20 points)}
    \begin{part}[a]
        \begin{answer}
            If we had not looked at the code of SquareTableElem, we would need 16 tests to cover all possible squares. Now we only need 4 tests 
            because there is only one if else statement on props.square.color == RED. And since the rest is straight line code we need two tests
            if it equals red and two tests if it doesn't.
        \end{answer}
       \end{part}

       \begin{part}[i]
        \begin{answer}
            Since we already have QuiltTableElem and everything else tested and we have two branches (QuiltTableElem and QuiltElem) and each branch returns
            a straight-line code. To test the code we changed in index.tsx, we need two tests if it includes table query and two tests if it doesn't (at least 4 tests in total). 
            In this case, we need to look at the patterns rendered with QuiltTableElemand check if they are consistent with the ones rendered with QuiltElem.
        \end{answer}
       \end{part}
\end{question}
...
\begin{question}{7. Extra Credit: Cycle-babble (0 points)}
    \begin{part}
        \begin{answer}
            \begin{align*}
                cycle(cons(a, L)) &= concat(L, cons(a, nil)) \tag{Def of cycle} \\
                \textbf{case1($L = nil$):}\\
                &= concat(nil, cons(a, nil)) \tag{Def of L} \\
                &= cons(a, nil) \tag{Def of concat}\\
                &\ne nil \tag{Since $a : \mathbb{Z}$} \\
                \textbf{case2($L \ne nil$):}\\
                &\ne nil \tag{Def of concat (concat two non-empty lists will never get nil)} \\
            \end{align*}
            Therefore, cycle(cons(a, L)) $\ne$ nil.
        \end{answer}
       \end{part}
       ...
       \begin{part}
        \begin{answer}
            \begin{align*}
                len(cycle(cons(a, L))) &= len(concat(L, cons(a, nil))) \tag{Def of cycle} \\
                \textbf{case1($L = nil$):}\\
                &= len(concat(nil, cons(a, nil))) \tag{Def of L} \\
                &= len(cons(a, nil)) \tag{Def of concat}\\
                &= 1 \tag{Def of len} \\
                &\ge 1 \\
                \textbf{case2($L \ne nil$):}\\
                &= len(concat(cons(b, R), cons(a, nil))) \tag{Def of L} \\
                &= len(cons(b, concat(R, cons(a, nil)))) \tag{Def of concat} \\
                &= 1 + len(concat(R, cons(a, nil))) \tag{Def of len} \\
                &\ge 1 \tag{len(L) $\ge$ 0}
            \end{align*}
            Therefore, len(cycle(cons(a, L))) $\ge$ 1
        \end{answer}
       \end{part}
\end{question}
